<template>
    <div class="row">
            ...
            <md-card-content>
                ...
                <md-list>
                    <md-list-item>
                        ...
                        <span class="md-list-item-text">{{user.surnames}}</span>
                    </md-list-item>
                    <md-list-item>
        ...
        <div class="col-lg-8 col-md-7 tabs-profile">
            <md-tabs md-sync-route>
                <md-tab id="tab-home" md-label="Inicio"  class="pb-5">
                    <div class="row d-flex justify-content-center">
                        <div class="col-md-6 col-12">
                            <h2 class="text-center pb-5 pt-5">¡Hola {{user.name}}! Bienvenido...</h2>
                        </div>
                        <div class="col-md-2 col-3">
                            <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 511.999 511.999" width="120">
                ...
                <md-tab id="tab-info" md-label="Información personal">
                    <md-field md-clearable v-bind:class="{ 'md-invalid': errors.first('name') }">
                        <md-icon>face</md-icon>
                        <label>Nombre</label>
                        <md-input required :disabled="sending" v-model="data.name" data-vv-as="nombre" v-validate="'required|min:5|alpha_spaces'" data-vv-name="name" maxlength="191"></md-input>
                        <span class="md-error">{{ errors.first('name') }}</span>
                    </md-field>

                    <md-field md-clearable v-bind:class="{ 'md-invalid': errors.first('surnames') }">
                        <md-icon>face</md-icon>
                        <label>Apellidos</label>
                        <md-input required v-model="data.surnames" :disabled="sending" data-vv-as="apellidos" v-validate="'required|min:5|alpha_spaces'"  data-vv-name="surnames" maxlength="191"></md-input>
                    ...
                    <md-field md-clearable v-bind:class="{ 'md-invalid': errors.first('email') }">
                        <md-icon>email</md-icon>
                        <label>Nombre</label>
                        <md-input required :disabled="sending" v-model="data.email" data-vv-as="correo" v-validate="'required|email'" data-vv-name="email" maxlength="191"></md-input>
                        <span class="md-error">{{ errors.first('email') }}</span>
                    </md-field>
                    <md-button class="md-raised md-primary" :disabled="sending || errors.any()" @click="updateInfo()">Actualizar datos</md-button>
                </md-tab>
                <md-tab id="tab-pass" md-label="Cambiar contraseña">
                    <md-field v-bind:class="{ 'md-invalid': errors.first('current_password') }">
                        <md-icon>lock</md-icon>
                        <label>Contraseña actual</label>
                        <md-input required type="password" v-model="pass.current_password" :disabled="sending" data-vv-as="contraseña actual" v-validate="'required|min:6'" data-vv-name="current_password" maxlength="191"></md-input>
                        <span class="md-error">{{ errors.first('current_password') }}</span>
                    </md-field>

                    <md-field v-bind:class="{ 'md-invalid': errors.first('new_password') }">
                        <md-icon>lock</md-icon>
                        <label>Nueva contraseña</label>
                        <md-input required type="password" v-model="pass.new_password"  :disabled="sending" data-vv-as="nueva contraseña" v-validate="'required|min:6'"  data-vv-name="new_password" maxlength="191"></md-input>
                        <span class="md-error">{{ errors.first('new_password') }}</span>
                    </md-field>

                    <md-field v-bind:class="{ 'md-invalid': errors.first('new_password_confirmation') }">
                        <md-icon>lock</md-icon>
                        <label>Repite la nueva contraseña</label>
                        <md-input required type="password" v-model="pass.new_password_confirmation"  :disabled="sending" data-vv-as="contraseña" v-validate="'required|min:6'" data-vv-name="new_password_confirmation" maxlength="191"></md-input>
                        <span class="md-error">{{ errors.first('new_password_confirmation') }}</span>
                    </md-field>
                    <md-button class="md-raised md-primary" :disabled="sending || errors.any()" @click="changePass()">Cambiar contraseña</md-button>
                </md-tab>
            </md-tabs>
        </div>
...
export default {
    ...
    data: () => ({
        ...
        updateInfo(){
            axios.put(`/admin/profile/updated/${this.user.id}`,this.data).then(response => {
                this.data = response.data;
                this.messageSnackbar = 'Exito. Se actualizo su información!';
            ...
            this.$validator.validateAll().then((result) => {
                if (result){
                    axios.put('/admin/changePassword',this.pass).then(response => {
                        this.pass.current_password = null;
                        this.pass.new_password = null;
                        this.pass.new_password_confirmation = null;
                        this.messageSnackbar = 'Exito. Se actualizo su contraseña!';
                        this.showSnackbar = true;
                        this.$validator.reset();
                    }).catch(err => {
                        if(err.response.status == 422){
                            this.$validator.errors.add({
                                ...
                                msg: err.response.data.current_password
                            });
                        }
                    });
                }
