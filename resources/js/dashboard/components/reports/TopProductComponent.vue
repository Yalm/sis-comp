<template>
    <md-card class="col-lg-8">
        ...
        <md-card-header>
            <div class="md-title">Top 10 de productos más vendidos</div>
        </md-card-header>
...
export default {
    ...
    mounted() {
        ...
        filterDate(){
            this.progress = true;
            axios.get(`/admin/reports/top?date_init=${this.date_init.toJSON()}&date_end=${this.date_end.toJSON()}`).then(response => {
                this.removeInfoChart();
                this.data = response.data;
                for(let element of response.data){
                    this.myChart.data.labels.push(element.name);
                    this.myChart.data.datasets.forEach((dataset) => {
                        dataset.data.push(element.TotalQuantity);
                    });
                }
                this.myChart.update();
                this.progress = false;
            ...
            pdfdoc.text('Top 10 de productos más vendidos',10,30);
            pdfdoc.autoTable({
                theme: 'grid',
                headStyles: { fillColor: [0, 0, 0]},
                columnStyles: {TotalQuantity: {halign: 'center'}},
                body: this.data,
                margin: {left: 10,top: 50},
                columns: [{header: 'Producto', dataKey: 'name'}, {header: 'Cantidad', dataKey: 'TotalQuantity'}]
            });
